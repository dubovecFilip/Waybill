...
namespace SCSSdkClient {
    ...
    public class SCSSdkTelemetry: IDisposable {
        ...
        // Fully qualified: with WinForms in scope, a bare "Timer" is ambiguous with
        // System.Windows.Forms.Timer, which would silently need a UI message loop.
        ...
        private void Setup(string map, int interval) {
            ...
            _updateTimer = new System.Threading.Timer(_updateTimer_Elapsed, null, tsInterval.Add(tsInterval), tsInterval);
#if LOGGING
        ...
        private void _updateTimer_Elapsed(object sender) {
            ...
            if (!scsTelemetry.SdkActive) {
                // Only re-arm the slower poll interval the first time we notice this,
                // but keep bailing out on every poll while inactive. The previous
                // "&& !paused" guard here only fired once: after that, paused stayed
                // true forever and this whole check was skipped, so a stale timestamp
                // sitting in shared memory (e.g. the game was killed instead of closed
                // cleanly) looked like a single new tick and then, since lastTime was
                // never advanced again either, nothing here caught it drifting further.
                if (!paused) {
            ...
            if (paused && scsTelemetry.SdkActive) {
                ...
                // Re-seed the baseline on reconnect too (e.g. the game was closed and a
                // new instance started later): flags left over from that other session
                // are not something that just happened in this one.
                firstActivePoll = true;
            }

            // These flags are sticky in shared memory: e.g. Fined/Refuel/RefuelPayed can
            // already be true on our very first read if that happened moments before this
            // process attached (or reattached after the game stayed open). Every diff check
            // below compares against fields that default to false, so without this the
            // first poll fires whatever was already set as if it had just happened. Seed
            // the baseline from the current state instead, with no events raised.
            if (firstActivePoll) {
